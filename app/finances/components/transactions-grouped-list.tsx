...
export default function TransactionsGroupedList({ data }: TransactionsGroupedListProps) {
    if (data.length === 0) {
        return (
        ...
        <Card className="shadow-md flex flex-col gap-4">
            <CardBody className="space-y-2 relative mt-3">
                {data.map((item, index) => (
                    <div
                        key={index}
                        className="group flex items-center justify-between p-3 rounded-xl bg-gradient-to-r from-white to-[#f9faff] border border-[#f0f4ff] hover:shadow-md hover:border-[#e4e9ff] transition-all duration-200"
                    >
